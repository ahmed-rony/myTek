...
function TableC({ closeNav, PropertyId }) {
  ...
  useEffect(() => {
   Card2_A5(PropertyId, setResponseState);
  }, []);

  let response = []
  const nullresponse = [
      { '#' : 0,  "Property Name": "Wait", [`Amount Charged ${getThisOneandTwoMonthBack()[0]}`]: "Wait"},
  ];
  ...
  if (typeof responseState === 'object') {
    ...
    response = [
       { '#' : 0,  "Property Name": "N/A", [`Amount Charged ${getThisOneandTwoMonthBack()[0]}`]: "N/A"},
    ];
  } else {
    response = [
       { '#' : 0,  "Property Name": "Error", [`Amount Charged ${getThisOneandTwoMonthBack()[0]}`]: "Error"}
    ];
  }
        ...
        <table className='tab_01' id='table-boards-11-tablecontainer'>
          <thead>
            <tr>
              ...
              <th className='table-boards-11-tablecontainer-thtd'>{`Amount Charged ${getThisOneandTwoMonthBack()[0]}`}</th>
            </tr>
          </thead>
          <tbody>
